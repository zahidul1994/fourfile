            ...
            <table class="striped responsive-table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="input-field right-align" ><input type="number" required name="collection" class=" right-align" id="collection"></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td><strong>Invoice No</strong><input type="text" id="invoicesl"></td>
                  <td><strong>Note</strong><input type="text" id="note"></td>
                  
            </tr>
              <tr>    <td></td>        <td class="indigo-text right-align" ><button class="btn indigo waves-effect waves-light page-footer" id="collectionsubmit">Submit</button></td></tr>
          </tbody>
            </table>
          </div>
          <!-- invoice subtotal -->
...
$(document).ready(function () {
  ...
  $.ajax({
          ...
          $('#monthlyrent').append('<span>' + data.result.bill[0].monthlyrent + '</span>');
          $('#due').append('<span>' + data.result.bill[0].due + '</span>');
          $('#intotal').append('<input type="hidden" value="'+data.result.bill[0].total+'" id="totall"/><input type="hidden" value="'+data.result.bill[0].id +'" id="collectionid" />' + '<span>' + data.result.bill[0].total + '</span>');
           // $('#search').val(' ');
          }
        });
}, 900));
...
$("#collectionsubmit").on('click', function(){
 ...
 if( $("#payby").val()==''){
  // console.log($("#collection").val());
   alert('Select Minimum One Pay Payby');
   $("#payby").focus();
   return false;
