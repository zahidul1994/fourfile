...
$(document).ready(function () {
  ...
  $.ajax({
          ...
          $.each(data.result.bill, function(key, value){
                       // alert(key);
                        $('#dd').append('<td>' + value.created_at + '</td><td>' + value.monthlyrent + '</td><td>' + value.addicrg + '</td><td>' + value.discount + '</td><td>' + value.advance + '</td><td>' + value.vat + '</td><td>' + value.due + '</td><td></td><td></td><td></td>'
                        );
                      //  console.log(value.collection.length);
                        if(value.collection.length>0){
                        $.each(value.collection, function(key, newvalue){
                        $('#dt').append('<tr><td>' + newvalue.updated_at + '</td><td></td><td></td><td></td><td></td><td></td><td></td><td>Collection By </br>' + newvalue.admin['name'] + '</td><td>' + newvalue.paid + '</td><td>' +((value.total-newvalue.paid).toFixed(2)) + '</td></tr>'
                        );
